...
export default function EmailInput({ onSubmit, loading }) {
  ...
  return (
    <div className="space-y-2">
      <form onSubmit={submit} className="relative cyber-input-form p-5 rounded-xl border-2 border-cyan-900 shadow-cyber bg-gradient-to-br from-[#101a24] via-[#071018] to-[#0b2730] text-cyan-200 font-cyber overflow-hidden animate-cyber-box">
        ...
        <div className="absolute inset-0 pointer-events-none z-0">
          {[...Array(12)].map((_, i) => (
            <div
              key={i}
              className="absolute top-0 left-0 w-full h-full animate-cyber-lines"
              style={{
                left: `${i * 8}%`,
                opacity: 0.06 + 0.03 * (i % 3),
                background: 'linear-gradient(180deg, #00fff7 0%, transparent 100%)',
                width: '2px',
                height: '100%',
                filter: 'blur(2px)'
              }}
        ...
        <div className="flex gap-3 items-center relative z-10">
          <div className="flex-1">
            <input
              className={`w-full bg-transparent outline-none placeholder:text-cyan-400 text-cyan-100 p-3 typing-placeholder rounded font-cyber text-lg tracking-wider shadow-cyber focus:ring-2 focus:ring-cyan-400 ${validationError ? 'border-red-500' : ''}`}
          ...
          <button
            type="submit"
            disabled={loading}
            className={`px-5 py-2 rounded font-bold text-lg shadow-cyber border-2 border-cyan-900 bg-cyan-800 hover:bg-cyan-600 transition-all duration-150 focus:outline-none focus:ring-2 focus:ring-cyan-400 ${loading ? "bg-gray-600 cursor-not-allowed opacity-70" : ""}`}
          >
            {loading ? (
      ...
      {validationError && (
        <div className="text-red-500 text-sm flex items-center gap-2 px-2">
