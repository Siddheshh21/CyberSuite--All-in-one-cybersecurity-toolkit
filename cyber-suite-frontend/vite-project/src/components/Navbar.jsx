        ...
        <div className="px-2 pt-2 pb-3 space-y-1 sm:px-3">
          {navItems.map((item) => (
            <Link
              key={item.path}
              to={item.path}
              className={`block px-3 py-2 rounded-md text-base font-medium ${location.pathname === item.path ? 'text-cyan-400 bg-gray-900' : 'text-gray-300 hover:bg-gray-700 hover:text-white'}`}
            >
              {item.label}
            </Link>
          ))}
